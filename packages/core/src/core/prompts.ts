...
export function getCoreSystemPrompt(userMemory?: string, skillsSummary?: string): string {
...
`.trim();
  ...
  const skillsSuffix =
    skillsSummary && skillsSummary.trim().length > 0
      ? `\n\n---\n\n${skillsSummary.trim()}`
      : '';

  return `${basePrompt}${memorySuffix}${skillsSuffix}`;
}
